...
def _Init():
    ...
    if system == "Windows":
        dll_candidates = [
            ...
            "libMVSDK.dll",
            "D:\\lib\\MVSDK.dll",
            "C:\\Program Files\\MindVision\\MVSDK.dll",
            "C:\\Program Files (x86)\\MindVision\\MVSDK.dll",
        ]
        last_err = None
        for dll_path in dll_candidates:
            try:
                _sdk = cdll.LoadLibrary(dll_path)
                CALLBACK_FUNC_TYPE = WINFUNCTYPE
                print(f"✅ MindVision SDK loaded: {dll_path}")
                return
            except Exception as e:
                last_err = e
        # If no dll found, _sdk remains None
        print(f"⚠️ Could not load MVSDK.dll. Last error: {last_err}")
        print("⚠️ MindVision SDK not found. Camera functions will not work.")
        CALLBACK_FUNC_TYPE = WINFUNCTYPE  # dummy
        return
    else:
        # Linux/Unix
        so_candidates = [
        ...
        for so_path in so_candidates:
            try:
                _sdk = cdll.LoadLibrary(so_path)
                CALLBACK_FUNC_TYPE = CFUNCTYPE
                print(f"✅ MindVision SDK loaded: {so_path}")
                return
            except Exception as e:
                last_err = e

        # If no so found, _sdk remains None
        print(f"⚠️ Could not load libMVSDK.so. Last error: {last_err}")
        print("⚠️ MindVision SDK not found. Camera functions will not work.")
        CALLBACK_FUNC_TYPE = CFUNCTYPE  # dummy
        return

_Init()
print("DEBUG _sdk =", _sdk)
#-------------------------------------------类型定义--------------------------------------------------
