...
class Bridge(QObject):
    ...
    def __init__(self, app_ref=None, tips_dir=GOOD_TIP):
        ...
        # Change this if ECON camera is index 1, 2, etc.
        self.camera_index = 1
    ...
    # --------- CAMERA SETUP ---------
    def set_exposure(self, camera, exposure_value):
        """
        Set OpenCV camera exposure.

        Note:
        CAP_PROP_EXPOSURE values vary by camera/backend.
        Test values such as -5, -6, -7 on Windows.
        """
        if camera is None or not camera.isOpened():
            print("⚠️ Camera is not open")
            return False
        ...
        try:
            ...
            print(
                f"[EXPOSURE] requested={exposure_value}, "
                f"actual={actual}, success={result}"
            )

            return result

        except Exception as e:
            print("OpenCV set_exposure error:", e)
            return False
    ...
    def _open_camera(self):
        ...
        try:
            print(
                f"🔍 Opening ECON camera index "
                f"{self.camera_index}"
            )

            if sys.platform.startswith("win"):
                # Recommended for USB cameras on Windows
                camera = cv2.VideoCapture(
                    self.camera_index,
                    cv2.CAP_DSHOW
                )
            elif sys.platform.startswith("linux"):
                # Recommended for ECON USB cameras on Linux
                camera = cv2.VideoCapture(
                    self.camera_index,
                    cv2.CAP_V4L2
                )
            ...
            if not camera.isOpened():
                print(
                    f"❌ Cannot open ECON camera index "
                    f"{self.camera_index}"
                )
                camera.release()
                return False

            # Set MJPG before resolution
            camera.set(
                cv2.CAP_PROP_FOURCC,
            ...
            # Reduce old buffered frames
            camera.set(cv2.CAP_PROP_BUFFERSIZE, 1)

            self.camera = camera

            # Give the ECON camera time to initialize
            time.sleep(0.5)

            # Remove initial unstable/buffered frames
            valid_frame = None
            ...
            if valid_frame is None:
                print("❌ Camera opened but frame grabbing failed")
                self.camera.release()
                self.camera = None
                return False

            self.latest_frame = valid_frame.copy()
            ...
            actual_fps = self.camera.get(
                cv2.CAP_PROP_FPS
            )

            print("✅ ECON OpenCV camera opened")
            print(
                f"📷 Resolution: "
    ...
    def _close_camera(self):
        try:
            ...
            print("📴 ECON OpenCV camera closed")
    ...
    def _grab_and_send_frame(self):
        ...
        if not self.camera.isOpened():
            print("⚠️ ECON camera is not open")
            return
        ...
        try:
            ...
            if not ret or frame is None:
                print("⚠️ OpenCV frame grabbing failed")
                return
    ...
    def startCamera(self):
        try:
            print(
                "▶️ startCamera() called - "
                "ECON OpenCV CAMERA MODE"
            )
            ...
            # Enable these checks when required
            # if machine_status() is False:
            #     return "plc_connection"
            ...
            if not self._open_camera():
                print("❌ Cannot start ECON camera")
                return "camera"

            # Show the initial frame immediately
            if self.latest_frame is not None:
                b64 = self._encode_bgr_to_base64(
                    self.latest_frame
                )
            ...
            # 33 ms gives approximately 30 FPS
            if not self.timer.isActive():
                self.timer.start(33)
            ...
            if (
                ...
                or not self.process_thread.isRunning()
            ):
                self.loops()

            print("✅ ECON camera live preview started")
            return "ok"
